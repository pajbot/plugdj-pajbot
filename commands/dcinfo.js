...
exports.handler = function (data) {
    ...
    User.find(user.id).on('success', function (dbUser) {
        ...
        if (isNaN(secondsSince(dbUser.last_leave))) {
            logger.info('/me ' + dbUser.username + ' has not disconnected in my presence.');
            return;
        }
        if (secondsSince(dbUser.last_leave) > settings['dctimer']) {
            logger.info('/me ' + dbUser.username + '\'s last disconnect (DC or leave) was too long ago: ' + sec_to_str(secondsSince(dbUser.last_leave)));
            return;
        }
        if (dbUser.waitlist_position > -1 && secondsSince(dbUser.last_leave) <= settings['dctimer'] && (position === -1 || (position > -1 && position > dbUser.waitlist_position))) {
            logger.info('/me ' + dbUser.username + ' disconnected ' + timeSince(dbUser.last_leave, true) + ' and should be at position ' + dbUser.waitlist_position);
            //move_user(user.id, dbUser.waitlist_position);
            //User.update({last_leave: null}, {where: {id: dbUser.id}});
        } else {
            logger.info('/me ' + dbUser.username + ' should not be moved.');
        }
    });
};
